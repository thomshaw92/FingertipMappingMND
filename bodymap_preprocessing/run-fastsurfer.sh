...
# Parse arguments.
PARSED=$(getopt --options "" --long sub:,sess:,session:,data-dir:,output-dir:,fast-dir:,dry-run,n-threads: --name "$0" -- "$@")
...
# Make sure to use the BASE_DIR value that was sepcified in the arguments.
if [[ -z "$FAST_DIR" ]]; then
...
INPUT_IMAGE="${SUB}/${SESSION}/anat/${SUB}_${SESSION}_acq-UNIDEN_run-1_T1w.nii.gz"
...
if [ "$DRYRUN" = true ]; then
    ...
    mv $OUTPUT_DIR/${SUB}_${SESSION} $OUTPUT_DIR/$SUB/$SESSION
    "
    echo "$SING_COMMAND"
else
    singularity exec --nv --no-mount home,cwd -e        \
