...
% Get thresh x and y coords
test_name = sprintf('test%d_coords', test_no);
...
if dets.output_by_fing == 1
    ...
    if sr2 == 1
        testnameadd = 'left_middle';
    elseif sr2 == 2
        testnameadd = 'left_index';
    elseif sr2 == 3
        testnameadd = 'right_index';
    elseif sr2 == 4
        testnameadd = 'right_middle';
    else
...
% Add to test name
test_name = [test_name, '_num', num2str(sr2), '_', testnameadd];

% Add vals to thresh var
if curve_fit_error(sr2,1) == 0
    thresh.(test_name) = [curve(sr2,i) dataStims_long(i)];
else
    thresh.(test_name) = [curve(sr2,i) dataStims(i)];
end

% Pull thresh for plotting reasons
this_thresh = thresh.(test_name);
...
if curve_fit_error(sr2,1) == 0
    if dets.fit_type == 1
        titletext = sprintf('%s - Learn test - Logit reg fit', sub);
    elseif dets.fit_type == 2
        titletext = sprintf('%s - Learn test - Weibull fit', sub);
    else
    end
else
